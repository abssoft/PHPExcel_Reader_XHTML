...
/**
 * PHPExcel_Reader_XHTML
 *
 * @category   PHPExcel
 * @package    PHPExcel_Reader
 * @copyright  Copyright (c) 2006 - 2012 PHPExcel (http://www.codeplex.com/PHPExcel)
 */
class PHPExcel_Reader_XHTML implements PHPExcel_Reader_IReader
{
	/**
	 * Input encoding
	 *
	...
	/**
	 * Create a new PHPExcel_Reader_XHTML
	 */
	...
	private function _flushCell(PHPExcel_Worksheet $sheet,$column,$row,&$cellContent,$checkNumeric=false) {
		if (is_string($cellContent)) {
			...
			if (trim($cellContent) > '') {
                ...
                /**
                 * set correct num format and autosize
                 */
                if ($checkNumeric && is_numeric(str_replace(' ','',$cellContent))){
                     $cell = $sheet->getCell($column.$row)->setValueExplicit(str_replace(' ','',$cellContent), PHPExcel_Cell_DataType::TYPE_NUMERIC);
                    $sheet->getStyle($column.$row)->getNumberFormat()->setFormatCode('#,##');
                    $sheet->getColumnDimension($column)->setAutoSize(true);
                } else {
                     $cell = $sheet->setCellValue($column.$row,$cellContent,true);
                     if (strlen($cellContent)>100){
	...
	private function _processDomElement(DOMNode $element, PHPExcel_Worksheet $sheet, &$row, &$column, &$cellContent){
        echo 'process: ',$row,',',$column, '<br />';
				...
				switch($child->nodeName) {
					...
					case 'p'  :
						...
						if ($this->_tableLevel > 0) {
                            ...
                            if ($row==1 && (in_array($child->nodeName,array('h1','h2','h3')))){
					...
					case 'td' :
                        ...
                        $this->_flushCell($sheet,$column,$row,$cellContent,true);
                        $span=intval($child->getAttribute('colspan'));

                        if ($span){
                            $sheet->mergeCells('A'.$row.':'.(chr($span+ord('A'))).$row);
                        }
